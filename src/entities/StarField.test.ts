...
describe('StarField', () => {
  test('should have 1500 stars', () => {
    ...
    expect(positions.length).toBe(GameConfig.starField.numStars * 3);
  })
  ...
  test('should recycle stars that are too far behind', () => {
    ...
    expect(positions[2]).toBe(halfSize + 1 - GameConfig.starField.fieldSize);
  })
  ...
  test('should recycle stars in X and Y dimensions', () => {
    ...
    starField.update(playerPosition);
    expect(positions[0]).toBe(halfSize + 1 - GameConfig.starField.fieldSize);
    ...
    starField.update(playerPosition);
    expect(positions[1]).toBe(halfSize + 1 - GameConfig.starField.fieldSize);
