...
- [2026-02-03 06:22:10] Apply `as const` to the `GameConfig` object in `src/config.ts` to enforce type-level immutability and prevent accidental modifications of game parameters.- [2026-02-03 19:22:42] Implement the `VisualComponent` interface for modular visual entity parts to ensure they expose a consistent `mesh` property.
